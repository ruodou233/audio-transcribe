...
把音频/播客/视频变成可靠文字稿的完整决策流程：**判断有没有现成稿 → 按质量要求与产出形态选路径 → 转写 → LLM 上下文校对**。三条路径：便宜线（免费单引擎快速稿）、标准线（说话人分离+结构化导出）、优质线（双 ASR 交叉验证+分歧裁决精确稿），实战验证过 3 小时双人访谈播客的产出。
...
- **三条命名路径**：便宜线（免费单引擎快速稿）、标准线（说话人分离+结构化导出，低价云端；分离标签可归正为真名，无法确证时保留标签）、优质线（双 ASR 交叉精确稿）——按质量要求与产出形态一次选对，不为不需要的精度付流程成本。
